...
const els = {
    connStatus: $('connStatus'), connText: $('connText'),
    modelChip: $('modelChip'),
    // 主题
    themeBtn: $('themeBtn'), themeModal: $('themeModal'), themeGrid: $('themeGrid'), themeCloseBtn: $('themeCloseBtn'),
    // 侧栏
    sidebar: $('sidebar'), sidebarMask: $('sidebarMask'), hamburgerBtn: $('hamburgerBtn'),
    personaName: $('personaName'), personaBadge: $('personaBadge'),
    personaDesc: $('personaDesc'), personaUpdated: $('personaUpdated'),
    editPersonaBtn: $('editPersonaBtn'), resetPersonaBtn: $('resetPersonaBtn'),
    personaAvatarBtn: $('personaAvatarBtn'), personaAvatarImg: $('personaAvatarImg'),
    profileCard: $('profileCard'), profileBody: $('profileBody'),
    profileUpdatedBadge: $('profileUpdatedBadge'), refreshProfileBtn: $('refreshProfileBtn'),
    diaryCard: $('diaryCard'), diaryBody: $('diaryBody'), diaryDateBadge: $('diaryDateBadge'),
    openDiaryBtn: $('openDiaryBtn'), writeDiaryBtn: $('writeDiaryBtn'),
    stBackend: $('stBackend'), stTurns: $('stTurns'), stAutosave: $('stAutosave'),
    stTokenText: $('stTokenText'), stTokenFill: $('stTokenFill'),
    saveBtn: $('saveBtn'), shutdownBtn: $('shutdownBtn'),
    // 聊天区
    messages: $('messages'), welcomeHero: $('welcomeHero'), welcomeAvatar: $('welcomeAvatar'),
    inputBox: $('inputBox'), sendBtn: $('sendBtn'),
    // 弹层
    avatarModal: $('avatarModal'), avatarGrid: $('avatarGrid'), avatarCloseBtn: $('avatarCloseBtn'),
    personaModal: $('personaModal'), personaInput: $('personaInput'),
    personaSaveBtn: $('personaSaveBtn'), personaCancelBtn: $('personaCancelBtn'),
    diaryModal: $('diaryModal'), diaryListPane: $('diaryListPane'), diaryReader: $('diaryReader'),
    diaryCloseBtn: $('diaryCloseBtn'), diaryRegenBtn: $('diaryRegenBtn'),
    shutdownModal: $('shutdownModal'), shutdownTokenInput: $('shutdownTokenInput'),
    shutdownConfirmBtn: $('shutdownConfirmBtn'), shutdownCancelBtn: $('shutdownCancelBtn'),
...
const THEME_NAMES = { coral: '珊瑚橙', mint: '薄荷绿', sky: '天空蓝', sakura: '樱花粉', lemon: '柠檬黄' };
...
function refreshAllAvatars() {
    [els.personaAvatarBtn, els.welcomeAvatar].forEach((el) => el && applyAvatarTo(el));
    // 消息流中的头像（历史与新增）同步刷新
    els.messages.querySelectorAll('.msg.ember .msg-avatar, .live-panel .msg-avatar, .typing .typing-avatar')
...
async function loadAvatars() {
    ...
    if (state.avatarFile && state.avatarList.length &&
        !state.avatarList.some((f) => f.toLowerCase() === state.avatarFile.toLowerCase())) {
        state.avatarFile = '';
...
function renderAvatarGrid() {
    ...
    if (state.avatarList.length === 0) {
        els.avatarGrid.innerHTML = '<p class="profile-empty">头像库还是空的（服务器未扫描到头像图片）<br>可将图片放入 agent\\web\\resource\\images\\avatars 目录</p>';
        return;
    }

    state.avatarList.forEach((file) => {
        const cell = document.createElement('button');
        cell.className = 'avatar-cell' + (file.toLowerCase() === (state.avatarFile || 'ember_default.jpg').toLowerCase() ? ' active' : '');
        cell.title = file;
        ...
        img.alt = file;
        img.loading = 'lazy';
        img.onerror = () => { cell.style.opacity = '.35'; };
...
async function api(path, options = {}) {
    ...
    if (data.code !== 0) {
        throw new Error(data.info && data.info.errorMessage ? data.info.errorMessage : (typeof data.info === 'string' ? data.info : '请求失败'));
    }
    return data.info;
}
...
function appendMessage(kind, text, { animate = true } = {}) {
    ...
    const avatar = kind === 'user'
        ? Object.assign(document.createElement('div'), { className: 'msg-avatar', textContent: '🧑' })
...
function createLivePanel() {
    ...
    head.innerHTML = '<span class="thinking-dots"><i></i><i></i><i></i></span><span class="thinking-label">正在思考…</span>';
...
function renderStatus(s) {
    ...
    const pct = s.maxTokens > 0 ? Math.min(100, Math.round((s.tokens / s.maxTokens) * 100)) : 0;
...
async function savePersona() {
    ...
    if (!text) { toast('人设描述不能为空', true); return; }
...
function renderProfile(p) {
    els.profileUpdatedBadge.textContent = p.updatedAt ? `更新于 ${p.updatedAt}` : '';

    if (p.isEmpty) {
        els.profileBody.innerHTML = '<p class="profile-empty">💬 多聊几轮之后，我会慢慢更懂你…</p>';
        return;
    }

    let html = '';
    if (p.summary) html += `<p class="profile-summary">${escapeHtml(p.summary)}</p>`;

    if (Array.isArray(p.traits) && p.traits.length) {
        html += '<div class="tag-group"><div class="tag-group-label">性格特征</div><div class="tags">' +
            p.traits.map((t, i) => `<span class="tag tag-trait" style="animation-delay:${i * 50}ms">${escapeHtml(t)}</span>`).join('') +
            '</div></div>';
    }
    if (Array.isArray(p.interests) && p.interests.length) {
        html += '<div class="tag-group"><div class="tag-group-label">兴趣话题</div><div class="tags">' +
            p.interests.map((t, i) => `<span class="tag tag-interest" style="animation-delay:${i * 50}ms">${escapeHtml(t)}</span>`).join('') +
            '</div></div>';
    }
    if (p.emotionalState) html += `<p class="profile-line"><span class="emoji-dot">🌤️</span>近期情绪：<b>${escapeHtml(p.emotionalState)}</b></p>`;
    if (p.communicationStyle) html += `<p class="profile-line"><span class="emoji-dot">💬</span>沟通偏好：<b>${escapeHtml(p.communicationStyle)}</b></p>`;

    els.profileBody.innerHTML = html || '<p class="profile-empty">💬 多聊几轮之后，我会慢慢更懂你…</p>';
}
...
async function loadDiaryCard() {
    try {
        ...
        if (d.exists) {
            ...
            els.diaryBody.innerHTML = '<p class="profile-empty">✍️ 今天的日记还没有写，聊过几句后我会自动动笔～</p>';
        }
    } catch {
...
async function loadDiaryList(selectedDate) {
    ...
    try {
        ...
        if (diaries.length === 0) {
            els.diaryListPane.innerHTML = '<p class="profile-empty">还没有日记，<br>先和我聊几句吧～</p>';
            els.diaryReader.innerHTML = '<p class="profile-empty" style="align-self:center;text-align:center;">还没有可阅读的日记</p>';
            return;
        }
        ...
        diaries.forEach((d) => {
            ...
            item.className = 'diary-item' + (d.date === (selectedDate || todayStr()) ? ' active' : '');
            item.innerHTML = `<div class="diary-item-date">📅 ${escapeHtml(d.date)}</div>` +
        ...
        // 默认选中（指定日期或最新一篇）
        const target = selectedDate && diaries.some((d) => d.date === selectedDate)
            ? selectedDate
            : (diaries.some((d) => d.date === todayStr()) ? todayStr() : diaries[0].date);
...
async function loadDiaryContent(date) {
    ...
    els.diaryListPane.querySelectorAll('.diary-item').forEach((it) => {
        it.classList.toggle('active', it.querySelector('.diary-item-date').textContent.includes(date));
    });
        ...
        if (h.messages && h.messages.length) {
            h.messages.forEach((m) => {
                if (m.role === 'user' || m.role === 'assistant') {
                    appendMessage(m.role === 'user' ? 'user' : 'ember', m.content, { animate: false });
                }
            });
            scrollToBottom(true);
        }
...
async function sendMessage() {
    ...
    try {
        ...
        if (hadLive) {
            ...
            await typewrite(bubble, r.reply || '（我没有想好怎么回复…可以再说一次吗？）');
        }
...
async function remoteShutdown() {
    ...
    if (!token) { toast('请输入关闭令牌', true); return; }
...
function bindEvents() {
    ...
    // 主题
    els.themeBtn.addEventListener('click', () => els.themeModal.classList.remove('hidden'));
    els.themeCloseBtn.addEventListener('click', () => els.themeModal.classList.add('hidden'));
    ...
    // 头像
    els.personaAvatarBtn.addEventListener('click', () => els.avatarModal.classList.remove('hidden'));
    els.avatarCloseBtn.addEventListener('click', () => els.avatarModal.classList.add('hidden'));
    ...
    els.diaryCloseBtn.addEventListener('click', () => els.diaryModal.classList.add('hidden'));
...
async function init() {
    ...
    bindEvents();
    await loadAvatars();
    await Promise.allSettled([loadPersona(), loadProfile(), loadHistory(), loadDiaryCard()]);
    els.inputBox.focus();
